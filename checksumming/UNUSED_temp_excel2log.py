...
if __name__ == '__main__':
    ...
    try:
        ...
        for ws in (wb[sheetname] for sheetname in sheets):
            ...
            for raw_row in ws.iter_rows(min_row=2, max_col=14, values_only=True): # 2018
                if raw_row[-1] != 'Secured Credit':
                    continue

                transaction = {k:v for k,v in zip(transaction_keys, raw_row[1:])}
                transaction['Raw ID'] = None if raw_row[0] is None else int(raw_row[0])
