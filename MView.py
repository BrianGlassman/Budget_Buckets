...
def make_sheet(parent, transactions: list[Record.CategorizedRecord], title: str, categories: tuple[str, ...]) -> None:
    values: dict[str, dict[int, float]] # {cat: {month number: value}}
    values = {cat:{m+1:0 for m in range(len(months))} for cat in categories}

    for t in transactions:
        if t.category not in categories: continue
    ...
    # Header row
    add_text(title.upper(), widths['label'], coords, bd=2, anchor='center')
    for month in months:
...
# make_sheet(root, categorized_transactions, "Income", Constants.income_categories)
make_sheet(root, categorized_transactions, "Expenses", Constants.expense_categories)
